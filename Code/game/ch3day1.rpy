...
label ch3day1:
    ...
    "Ну и громадина!"
    if gunPoints>=2:
        "Надо что-то предпринять, срочно. Спрячусь под машину — умру. Побегу по прямой — умру."
        "Уйти из угла обзора! Я резко прыгнул влево, побежал между машинами пригнув голову."
        "Монстр принялся крушить всё на своём пути, к счастью его путь не пересекался с моим."
        "— Есть кто на связи?"
        sl "— Я тут!"
