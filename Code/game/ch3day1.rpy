...
label ch3day1:
    ...
    menu:
        ...
        "Посмотреть налево":
            ...
            if playedChess:
                ...
                "Я посмотрел налево и увидел парня со странной причёской."
			# bulka: Я не помню, игра была обязательной или нет. Если да, то думаю сделать необязательной. Я уже сонный, а потому переработка, скорее всего, будет в другой раз. Сегодня я не уложился во временной лимит мозговой активности. Сори.
            ...
            pav "— Павел. Будем знакомы. Хотя, ты мне не особо интересен."
            "— Это ещё почему?"
            pav "— А чем ты можешь быть мне полезен?"
            "Я даже не знаю, что и ответить на такое."
            pav "— Ладно, не парься, просто было интересно, как ты отреагируешь."
            "Павел слегка усмехнулся."
            pav "— Этот координатор... Готов поспорить, он уже забыл, что хотел от нас."
            if wonChess:
                "— Он не особо умён, согласен. Но не думаю, что настолько."
                pav "— А ты с ним уже знаком?"
                "— Да, вчера встретился с ним."
            else:
                "— С чего такие поспешные выводы?"
                pav "— Я вижу людей насквозь."
            "Павел стал всматриваться в меня. Он что, буквально пытается просмотреть меня насквозь?"
            if inferno:
                pav "— Так. А ты не так прост как кажешься на первый взгляд. Уже убивал?"
                "Меня бросило в холодную дрожь от такого вопроса."
                "— Нет, с чего бы?"
                pav "— Точно убивал. И очень недавно."
                "Да кто он такой чтобы читать меня?"
                "— Опять шуточки свои?"
                pav "— Раскусил! Ты аж побледнел, как будто и правда кого-то грохнул."
                "Павел снова усмехнулся. Не верю, что это была просто шутка."
                pav "— Как ты сюда попал?"
                ...
                pav "— Я смотрю в тебя и не вижу совсем ничего интересного. Ты какой-то скучный."
                "Павел продолжал смотреть в меня. Как же он давит! И как мне с ним сражаться в одной команде?"
                pav "— Ты чего такой серьёзный-то? Я же просто шучу..."
                pav "— Как ты сюда попал?"
                "Я рассказал ему вкратце про кораблекрушение. Всего ему знать не следует, не доверяю я ему."
            
            pav "— Прохладная история." 
            "— Не зли меня." with bloodrage
            pav "— Ладно, проехали. Расскажу о себе."
            pav "— У меня всё гораздо проще, плановая проверка на вирусы. Якобы лет 20 назад была утечка какого-то штамма ВИЧ, и нужно было всех проверить. На самом деле проверяли способность управлять нейронтами. Кстати, я слышал, что это технология Инферно."
            "— Возможно..."


    # bulka: Какой-то Павел слишком другой. Это он же у нас вроде как Павлик Морозов планировался? Мне что-то казалось, что он какой-то трусливый и неуверенный в себе будет.
    ...
    "Словом, ничего особенного. Но к себе в комнату я вернулся уставшим, как собака."
