label ocean:
    ...
    # scene bg sky
    with fade
    nvlc "Я открыл глаза.{w} Всего лишь небо и больше ничего. Слепяще белые облака. Ярко голубое пространство, простирающееся на миллиарды километров ввысь. И звук ветра, затихающий и еле заметный. Я мог бы долго так лежать и любоваться красотой бесконечной глади океана, если бы не одна любопытная мысль, пришедшая мне в голову."
    # bpaw: космический океан?
    # bulka: В самом деле не очень звучит. Космическое море мб тогда?
    # bpaw: может так?
    # pomis: нормас
    nvlc "А где я?"
    nvlc "Впрочем, мысль я особо развивать не стал. Кругом так безмятежно, тихо и спокойно, что глаза сами закрылись. Ничего не мешает поспать ещё часок, разве что солнце припекает, да и покачивает меня почему-то...{w} Опять качнуло.{w} И вдруг я вспомнил..."
    nvl clear
    with vpunch
    ...
    # bpaw: Кстати, Юминг нифига не похожа на азиатку. Может сделать её кожу более смуглой?
    # bulka: Можно глаза в пол хлебала сделать. Тогда никто сомневаться не будет.
    # pomis: Она будет перерисована в любом случае.
    "— Эй, ты как?"
    "Надо помочь ей?"
    ...
    # bpaw: Нет, пусть будет несколько имён.
    # bulka: *facepalm*
    "Чёрт побери. Как же меня зовут? Голова раскалывается..."
    menu:
        "Пётр":
        ...
        "Максим":
            ...
            # bpaw: Минус одна агрессия, как заказал булка
            # bulka: Сказал он и увеличил вероятность попадания в инферно рут через убийство вдвоеXD
            # pomis: Свободная касса!
        "Алексей":
    ...
    if playerName=="Алексей":
        $ yumingPoints += 1
        # bulka: Шкуру именем цепляемXD
        yu0 "— Знала я одного Алексея...{w} А? На том самом корабле?"
        # bulka: Интересно, мы об одном и том же Алексее подумали?
        # pomis: Думаю, нет. А какой у нас общий знакомый Алексей? Бликов, что ли? Точно нет)
    else:
    ...
    # bulka: Учимся развивать воображение.
    # bpaw: Какое может быть воображение у игроков? Они все как амёбы
    # bulka: Если я скажу, что в такие игры вообще играют люди не обременённые интеллектом, то кого я оскорблю больше: игроков, что будут играть в эту игру или нас, что эту игру создают?
    ...
    # bulka: Не уверен, что эта даблъю тут нужна.
    "Вслух? Я это вслух сказал? Или показалось?"
    yu "— Ну, конечно, сломано, ты посмотри на вмятину! Ничто не вечно при крушении. Корабль вообще утонул, хотя планировалось, что он обойдётся без жертв."
    "— А какое отношение имел корабль к вашей операции?"
    yu "— Ну-у, так вышло, что мы брали их на живца..."
    # pomis: вот тут она прям грузит игрока ↓ да и не в её характере много рассказыватб, переделал
    # yu "— Таких так ты у нас называют «способными», поэтому ты имеешь право знать больше информации."
    # "— Какой информации?"
    ...
    if agressionPoints>1:
        ...
        jump rootSoldier
        # bpaw: реквестирую такую фигню: когда начинается новая глава - показать понтовую надпись на весь экран (номер главы и название)
        # bulka: Отличная идея! Поди да сделай!
        # pomis: Как в бесконечном лете и катаве? Да, обязательно надо. Я хотел их сделать после того, как будет полностью написан сюжет, чтобы схематично показать игроку, в каком ответвлении он находится.
        # bpaw: Возникла идея тяжелее: эти фигни, которые вы поддерживаете, сделать как в The World Ends With You. Вот ссылка на фастпик tinyurl.com/twewy-chapter-example
        # bulka: Тут уже вопрос к Роману. Не знаю возможностей движка, но если он поддерживает анимацию, то вряд ли будет проблемой в начало файла зафигачить строчку, отображающую анимацию.

    # bpaw: белеберда снизу — это что?
    # pomis: Старая версия, то, что жалко было удалять
    # bulka: Эта глава столько раз передывалась, что просто ужас. После реворка убийства мне больше не хотелось открывать первые главы, но я всё же лелею идею, однажды, взяться и привести их в порядок. Хотя, если это сделает кто-то другой, то я не против.
