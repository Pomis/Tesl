...
label ch3day2:
    ...
    if result == "tir":
        ...
        if inferno:
            ...
            "— Кому?"
            pav "— Организации TESL. А именно, руководству этой базы. Это они уничтожили мой дом! Мой городок! А всё свалили на газовую аварию!"
            ...
            if stangerPoint>0:
                            ...
                            if tenkoAttention>1:
                                ...
                                menu:
                                    "Признаться":
                                        ...
                                        # bulka: Была мысль, что при наличии пойнтов появится Мария/Минори и застрелит Тенко, но понял, что это уже будет слизано с гиасса. Как же всё таки меня прёт с этой анимешки.
                                        if PavelHelp:
                                            $ PavelHelped = True
                                            "Вдруг раздался выстрел. Из вентеляции выглянул Павел с автоматом наперевес."
                                            ...
                                            # bulka: И ЭТО РЕАЛЬНО ПРОБЛЕМА. Я ЖЕ ЕГО НИЧТОЖЕСТВОМ ХОТЕЛ СДЕЛАТЬ. ХУЛИ ОН КРУТЫМ ВЫХОДИТ\
                                            # bulka: Ну и бог с ним. Не буду зацикливаться не стереотипах. 
                                            ...
                                            "Тенко отошёл и я полез за Павлом обратно. Так-то!"
                                            # bulka: Да и там дальше разговор с Павлом с этим не сочетается.
                                        else:
                                            ""
                                            # bulka: Итак, что же тут делать? Вводить отдельный рут?
                                    "Отрицать":
                ...
                # Если не накоплено ни одного antiTenkoPoints
                    ...
                    "— Как же ты достал..."
                    # bulka: Хз, но думаю это будет тупик. Хотя это будет немножко глупо. У нас тупики, кстати, планируются?
                    # pomis: Охренеть ты тут вариантов сделал) Какой ещё тупик? О_о
                    # bulka: В смысле конец. Иначе я хз, что делать, кроме отдельного рута пленника/ещё кого.
                    # pomis: забей на тупик. Тут надо линейно вывести всё дальше.
                    # bulka: Тогда тут будет какое-то ранение гг. Или Тенко жучок на него повесит.
                    # bulka: Неа. Не ебу пока что тут делать. I don`t know what must be here, bleat`!
                    # pomis: сложнааа
            ...
            pav "— Вас понял!"
            # bulka: Тут я понял, что у меня есть некие проблемы понимания в устройстве острова и куда шахта ведёт, а потому я не мог продолжить. За сим призываю сделать фрагмент до их лодки тебя.
            ...
            "Мы спустились по крутому склону и вышли к берегу. Как-ни странно, рядом красовалось небольшое судно. Думаю, они просто знают позицию Павла."
            # pomis: как-то так. 
            "Честно говоря, я ожидал нечто более футуристичное."
            menu:
                "— И куда мы на ней поплывём?":
                    $ infernoChoToTam = True
                    #bulka: Тут будет какая-то переменная, означающее знание о технологиях Инферно/ещё чего.
                    pav "— Ты слышал об обратной зависимости крепкости сна от количества знаний?"
                    "— Шутки смешны лишь тогда, когда они уместны."
                    pav "— Нам надо доплыть до точки, где они нас смогут подобрать на борт."
                    ...
                    #bulka: Тут должно быть условие/меню, в одном из случаев дающее знание о месте назначения knowCoordinates. Это потом облегчит попадание к Инферно/ещё чё. 
                "— Ты умеешь управлять лодкой?":
                    pav "— Не думаю, что это будет трудно."
                    #bulka: Что-то с этим предложением не так, но что именно - хз
                    # pomis: сложноватое и запутанное
                    "Оптимистичное заявление..."
                    pav "— Насколько мне известно, управлять ею не сложнее, чем автомобилем."
                    # pomis: с этими предложениями что-то точно не так
                    "Значит, если что, я тоже смогу управлять ею."
                    #bulka: Такое ощущение, что я пишу диалог с Марией, а не Павлом.
            "Мы забрались в лодку и Павел завёл двигатель."
            if infernoChoToTam == False:
                "Оказалось, что управление в самом деле не было сложным."
            jump ch3day3
        else:
