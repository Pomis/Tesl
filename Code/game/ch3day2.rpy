...
label ch3day2:
    ...
    if result == "classes":
        ...
        if inferno==True:
            ...
            menu:
                "А мне кажется, это люди во всём виноваты!":
                    ...
                    # bulat: Или у инферно, если срединный союз придётся убрать.
        ...
        ma "— Нет конечно. У нас много военного персонала, не обладающих открытой нервной системой. Без обычных войск никакую войну не выйграть. Соответственно, их никто в ангар не пустит."
        ...
        "Тут Павел стреляет из Ак-12 и рассказывает свою историю. "
        jump hangardefault
...
label hangardefault:
    ...
    if withMarieInHangar==True:
        ...
        if inferno == False:
            ...
            "Мы помогли новобранцам залезть и запустить их боевые машины, однако когда мы закончили, прогремел уже четвёрный удар. "
            sl "— Быстрее!"
            $ slavaAlive = False
    ...
    # bulka: Эх, а я тут идею о побеге с базы делал. Слишком быстро развивается сюжет. Ну да ладно, сойдёмся на том, что будет возможность вернуться на базу.
    # Правда всё равно не понимаю, как в таком случае он присоединиться к инферно. Начнёт из своей лодки палить по другим? Или просто свалит? Как я понял лодка может быть заблокированна удалённо.
