...
label warBase:
    ...
    if inferno==False:
        ...
        "Белым существом оказалась блондинка в белом халате."
        # bulka: Почему убрал определение? Прилагательные - очень хорошее подспорье в повествовании и внушении чего-либо. Офк, написано было с ошибкой, но словом "симпатичная" мы внушаем игроку, что Мария в самом деле симпатичная, а не просто заставляем его оценивать рисунок.
...
label goHome:
    ...
    if yumingAlive:
        ...
        if askedAboutCopter == False:
            menu:
                "— Можете рассказать про свой вертолёт?":
                    $ askedAboutCopter = True
