...
label earth:
    ...
    port "— Проводили исследования по изменению судьбы. На людях. Но пришлось свернуть деятельность из-за огромной ресурсоёмкости и опасности проекта."
    # bulka: Кажется, ты слишком буквально воспринял название новеллы. Я, конечно, не против, если это будет важной частью сюжета, но научный реализм.
    # pomis: Я попытаюсь придумать псевдонаучное объяснение всему этому. Да и мы уже и так немного отошли от научного реализма
    "— Как это вообще возможно? Получилось что-нибудь?"
    port "— Нет, только куче людей испортили жизнь..."
    if portPoints>0:
