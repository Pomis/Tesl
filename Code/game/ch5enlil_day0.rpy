...
label ch5enlil:
    ...
    if stolenFlash:
        ...
        sa "— Да ты сам себе противоречишь. Как нам его переманить на нашу сторону не разговаривая с ним на одном языке? Может, ему выучить язык Инферно? Даже Алилум смог освоить два людских языка!"
        "Судя по выражению лица Уша, он был готов убить сенатора прямо сейчас."
    ...
    menu:
        "— У меня дела поважнее.":
            ...
            if stolenFlash:
                ...
                alil0 "— Зачем тебе вообще есть? Совсем очеловечилась?"
                "— Нам привезли целую партию бананов, не пропадать же добру!"
                alil0 "— Ладно, мне всё равно не хочется кого-то искать, подбирать для задания. В общем, нужно проследить за каким-то там человеком."
                jump mission
            

        "— Ну, выкладывай!":
            alil0 "— Я сюда зашёл только потому, что это здание находится возле моего дома. Поручение не особо важное, нужно проследить за каким-то там человеком."
            jump mission
...
label mission:
    ...
    "— Энлиль."
    alil "— Значит так, Энлиль, если будет что-то важное, в чём я очень сомневаюсь, сообщай мне. Ах да, держи пропуск в портал на поверхность в одну сторону."
    # bulka: Почему в одну?
