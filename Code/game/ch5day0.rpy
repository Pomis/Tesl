...
label ch5day0:
    ...
    label missionShore:
        gen "— Итак. Нас проинформировали о планируемом нападании Инферно на этот город. Судя по всему, они узнали, что мы тут находимся, и решили стереть этот город с лица земли."
        gen "— Замечено передвижение в океане двух групп существ в сторону нас. Мы не знаем точно, какие это существа, но судя по их тактике, скорее всего, часть будет оставаться под водой и ретранслировать энергию с их базы атакующим существам."
        gen "— По этому, мы разделимся на две группы, одна будет сдерживать наступление, другая уничтожит подводных существ."
        pi "— А как мы разделимся?"
        gen "— На подводную миссию отправим только самых опытных пилотов. Нам нужно много рук, чтобы отбить нападение на город, а чтобы уничтожить ретрансляторы хватит небольшой группы."
        "Моё внимание привлёк листик бумаги, явно выпавший откуда-то прямо под мою табуретку. Подниму его как только будет удобный момент."
        nvl clear
        nvlc "Ох, как же кушать хочется... Мой пустой желудок прервал их беседу своей голодной нотой. Вскоре, нас отпустили, предупредив, что через два часа быть готовым к миссии."
        ...
        if iscraPoints>1: # Если есть хоть немного положительного взаимодействия с Искрой
            isc "— Чем занят?"
            "Я слегка вздрогнул от неожиданности. Искра взяла бумажку у меня из рук."
            isc "— Хм-м... Ты что, без рецепта даже плов приготовить не можешь?"
            "— Да это не моё..."
            isc "— Самое нелепое оправдание, которое только можно придумать. Ладно, пошли, приготовлю тебе это. А то песня твоего желудка меня напугала больше, чем сказки генерала."
            menu:
                "Дожидаться Славу":
                    $ selectedIscraDinner = False
                    if iscraPoints > 4:
                        # isc "— Нет, ты идёшь со мной." #pomis: нет, но оставлю на всякий
                        isc "— Думаешь, он умеет готовить что-то кроме перловки?" 
                    else:
                        isc "— Ну, уговаривать тебя я не буду. Жди своего солдатика."
                "Пойти с Искрой":
                    $ selectedIscraDinner = True
                    isc "— Хорошо, у тебя или у меня?"
                    "— У меня нет продуктов..."
                    isc "— А ты в шкафчиках на кухне смотрел? Это же квартиры на случай чрезвычайного положения, в них запасов крупы на пару месяцев."
                    "Честно говоря, я даже не пытался что-то в них искать, Слава, видимо, тоже не самый любопытный человек. Как-то неудобно получится, если не предупредить его."
        sl "— Думаю, нам хватит двух килограмм картошки?"
    ...
    label missionMoscow:    
        ...
        "— Слава, одолжу у тебя толстовку?"
        if slavaPoints>0:
            sl "— Да без базара, бери!"
        else:
            sl "— Должен будешь."
        "— Спасибо."
        ...
        # они уже под контролем, всё, что ниже — обыгрывание смерти, которое им внушено, чтобы убедить в реальности сна.
        ...
        "Может, я смогу заговорить с ними?"
        "— Мы не знаем ни одной другой базы кроме той, что вы уничтожили!"
        isc "— Вот именно!"
        no1 "— Так, кажется у некоторых из вас хороший иммунитет! Убейте их!"
        "Горячая пуля пронзило моё плечо, затем две пули прошили мой живот, а последняя попала прямо в лоб."
    ...
    # pomis: Возьму тут за основу законсервированную версию ch6enlil

    # pomis: Пока мы не определились с глобальной концепцией, делать встречу отряда школьников и инферно не стоит. Добавлю промежуточную миссию. Примерные пункты по этой миссии:
        # Пусть они стреляют из огромных пушек по гигантам
        # Нужно рассорить школьников
        # Нужно показать, что инферно обладают чувствами и собственной позицией
            # bulka: Инферно или инферналы?
            # pomis: первое было везде
        # Нужно учесть что в руте с Марией обыгрывалось, что до Юминг невозможно дозвониться
    jump ch5nightmare
