label ch3day3:
    ...
    # pomis: Я как-то делал фон, где показывалась база. Там не совсем остров был. Сохраним смысловое единство.
    # bulka: https://pp.vk.me/c631531/v631531897/7c97/HhpubGXaXlA.jpg Напоминало остров
    ...
    # pomis: сначала говорит про море, потом про океан — не порядок. Да и слова типо «собственно» — словесный мусор, постараюсь не использовать их, и уберу там, где они не несут смысловой нагрузки.
    # bulka: Почему мусор-то?   
    nvl clear
    call flashback4
    if vpunch:
        nvlc "До сих пор помню тот отвратительный вкус брокколи. И почему мне это вспомнилось?"
        nvlc "Хотя почему-то от этого воспоминания на сердце стало теплее."
    else:
        nvlc "Какой же мразью я был...{w} Именно поэтому, хотя бы это я должен для неё сделать. Хотя бы в этом я должен быть братом!"
    ...
    "Мы же уплывали в неизвестном направлении. Надеюсь, на корабле нас не заметили."
    # pomis: мы уже в отрытом море (океане).
    if PavelUseless:
    ...
    menu:
        "Попробовать активировать его.":
            $ debilPoints += 1
            "Я попробовал активировать его. Он явно работал, но не выполнял своей функции.{w} Такое ощущение, будто звонишь по телефону, но никто не берёт трубку. Видимо, это средство связи."
        ...
        "Спросить Павла.":
            ...
            "Я аккуратно положил передатчик на место.{w} Хорошо, что я не стал проверять что это."
        "Положить обратно":
    ...
    "Наконец показалась земля. Остров или материк это был, я не знал. У меня с детства с географией было туго. А вот у моего соседа по парте и друга по совместительству с этим проблем не было."
    ...
    call flashback6
    "Мда. Как говорится: оценки в жизни не помогут. Вот так и получилось."
    # bulka: Или "оценки в жизни не помогут". Хз какой вариант лучше.
    # bpaw: Вариант про оценки лучше
    # bulka: Хотя тут немножко бредово выходит. ГГ говорит, что у него с географией было туго, то бишь оценки фиговые, а потом говорит, что они в жизни не помогут, когда тут бы это пригодилось...
    ...
    # bpaw: В тексте постоянно попадаются и нейронты, и нейроинты. Что есть верно?
    # bulka: Думаю и надеюсь, что второе. Но это не точно. Это придумал Рома, так что ему лучше знать.
    # pomis: Я был уверен, что их придумал Оскар
    # bulka: Разве? Ну возможно. Суть в том, что я в их создании участия не принимал.
...
label debil:
    ...
    # bulka: Ну или не в следующей. Филлеры никто не отменял.
