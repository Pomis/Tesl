...
#    "— Посмотри в том кей..."
    ...
    nvlc "Оставшаяся ночь была самой кошмарной за всю мою жизнь — я остался один на один со своей совестью. {w}Хоть я и был атеистом, мне казалось, что теперь мне обязательно вернётся моё зло."
    nvlc "И даже аргумент, что из-за Юминг погибли люди был уже неубедителен для меня." 
    hide boat onlayer middle
    scene black onlayer background with closing
    nvl clear
            ...
            "— Да я так, просто...":
                ...
                menu:
                    "Согласиться":
                        ...
                        "Лёг спать примерно в полночь, ставлю будильник на электронных часах в палате."
                        "Утром направился сдавать анализ."
                        jump analysis
...
label fuckRoommate:
    ...
    # pomis: согласованность времён полетела
    ...
    if agressionPoints<4:
        "Хотя, я и сам был не особо вежлив с ним."
    "Может..." 
...
label ihateu:
    ...
    "В первую очередь нужно добыть самое сложное — средство убийства. Без этого весь план полетит к чёрту."
    ...
    # bpaw: Зачем эта if-конструкция? 
    # bulka: Чтобы бы млять слова и действия в случаях убийства и подставы различались. Очевидно же. Или два куска текста сравнить перед удалением не судьба?
    ...
    nvl clear
    "В первом отделении лежали предметы, что можно найти в каждой аптечке. Ничего интересного."
    ...
    if wantToKillNeighbour:
        ...
        "Ведь кто-то просто так уничтожил мою семью. Это же даёт мне полное моральное право убивать тех, кто не неугоден?"
        "Думаю, да."
        "Эйфория от свершённой мести царила моим сознанием ещё примерно час. Я просто ходил вокруг корпуса и радовался своим величием."
    ...
    # Отслыка к Akame ga Kill, там серии имеют названия вроде «Убить сестру»
    # bulka: Больше похоже на "как достать соседа".
    # pomis: Ньюфаги не поймут
    nvl clear
...
label nextDay:
    ...
    if brokenFinger == True:
        ...
        # bpaw: Тут нужно описание их общения, ибо они все по инглишу не ту-ту
