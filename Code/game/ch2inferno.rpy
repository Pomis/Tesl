...
#    "— Посмотри в том кей..."
    ...
    # bulka: Тут нужно дать одно очко $ killPoints += 1, но так как я щас буду заниматься другим, а дальше от этих очков будет что-то зависеть и надо будет исправлять, то просто оставлю это в комментариях. 
    # pomis: судя по тому, как эти очки добавляются, это скорее навык обращения с оружием. Тут же введу счётчик убийств killedPeopleCount, который будет влиять на сожаления, хладнокровие при убийстве итд
