...
label ihateu:
    ...
    nvlc "Когда я ходил в туалет (поиск — целая эпопея), я увидел тележку с лекарствами."
    if $ killN == True:
    ...
    nvlc "На ручках отделений были замки, но кто-то не очень умный забыл ключи в замке первого ящечка.{w} Мысленно поблагодарив этого недалёкого человека, я начал осмотр содержимого."
    nvlc clear
    ...
    nvlc "Уничтожить."
    nvlc clear
    # Ещё одна. Сюда какой-нибудь эффект типа БАБАХ или БЫДЫЩ
    if agressionPoints>3:
        ...
        if playerName == "Илья":
            # Убийство при помощи водки, только для Ильи
            jump killingNeighbourIlya
        else:
            # Убийство при помощи чая, для всех, кроме Ильи
