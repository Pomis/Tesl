...
label killing:
    ...
    nvlc "Верно! Чего этот сосед возомнил о себе! Биомусор. Я должен убить его. Земле-матушке и без них тяжело... А с ними ещё тяжелее."
    nvlc "Наверное, тяжелее. Кто эту Землю знает?"
    nvlc "Как я уже говорил, помнится, тут была тележка с лекарствами..."
    nvlc "Почти весь остаток дня я провёл в палате, старательно делая вид, что не обращаю внимание на моего соседа."
    nvlc "Однако, я всё-же наблюдал за ним."
    nvlc clear
    ...
    if playerName == "Илья":
        ...
        $ drink = "чай"
    scene bg hospital room sunset onlayer background
    nvlc "Мой сосед, как и подобает русскому человеку, очень любит [drink]. Это можно использовать."
    # bulka: С КАКИХ ПОР РУСССКОМУ ЧЕЛОВЕКУ ПОДОБАЕТ ПИТЬ ЧАЙ? АНГЛИЙСКОМУ ЭММИГРАНТУ ШОЛЕ?
    nvlc "Когда я ходил в туалет (поиск — целая эпопея), я узнал точно, где можно найти лекарства."
    # bpaw: Это можно описать
    nvlc "Они были около туалета."
    nvlc "Внимательно изучив содержимое тележки, я узнал, что там есть медицинский спирт, уксус, местные аналоги парацетамола и прочих смект, много разных верёвок и бинтов."
    nvlc "Нужно искать самые мелкие лекарства. Такие требуют очень точной дозировки."
    # bpaw: Нафига уксус? Илья им убивает. Нужно оправдать наличие уксуса среди лекарств.
    # bulka: Эм, что? Его, как бы, в качестве приправы используют, как ты им убивать собрался? 
    # bulka: И как он знает, что это аналоги парацетамола? Как бы с испанским у него туго, знания английского тоже не на высоте.
    nvlc "Как мне это может пригодиться?"
    nvlc "Когда я пришёл в палату, этот гад улёгся спать."
    nvlc "Убить соседа."
