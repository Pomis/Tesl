...
label warBase:
    ...
    if wantToAskAboutSignal == True:
        ...
        ma "— Ну и зверство! Вы что, были наживкой? Зачем ты в этом устаствовал? Я думала, ты умнее."
...
label goHome:
    ...
    "Я вошёл в свою комнату. Довольно тесноватая, как в общежитии. Искусственное окно. Соседей не было, хотя стояло три кровати."
    if inferno == True:
    ...
    menu:
        ...
        "Прогуляться по жилому комплексу":
            ...
            "На диванчике расположился молодой человек с забавной причёской. Он разложил на шахматной доске фигуры в случайном порядке и перемещал их."
            ...
            if knowYumingRoom == True:
                ...
                menu:
                    "Зайти к Юминг":
                        ...
                        "----тут они миленько болтают, а потом гг идёт к себе----"
                    "Подойти к гроссмейстеру.":
                        jump grossmeister
            else:
                jump grossmeister

            # Если знает где живёт Юминг — выбор, идти к ней, или дальше гулять
...
label grossmeister:
    ...
    "— В чём смысл?"
    co0 "— Нет смысла. О! А я тебя никогда не видел. Хм. Новобранец? Нет. Тогда бы ты был в корпусе новобранцев..."
    "Он знает про лестницу?"
    co0 "— Ладно, не важно, кто ты. Важно, кто я."
    co "— Я координатор новобранцев, а зовут меня Жорик. "
    "— А я [playerName]."
    co "— Не хочешь сыграть партию?"
    "Так, значит он будет моим координатором? Наверное, лучше не отказывать ему."
    "— Хорошо, давай сыграем пару раз..."
    "Это было очень легко. Я даже смог поставить ему \"детский\" мат. Единственная стратегия, которую я помню."
    co "— Это нечестно! Отставить думать! Играй по-мужски!"
    "Пришлось проиграть ему один раз, чтобы он успокоился. "
    co "— Вот, другое дело! Ладно, теперь я буду спать с чувством выполненного долга!"
    if inferno:
        "С такими кадрами у людей нет шансов"
    else:
        "С такими кадрами у нас нет шансов"
    "— Спокойной, я тоже пойду."
    "Я вернулся в комнату, Слава в то время уже спал богатырским сном. Я плюхнулся в кровать и отключился."
    jump ch3day1
